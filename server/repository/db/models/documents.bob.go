...
var Documents = mysql.NewTablex[*Document, DocumentSlice, *DocumentSetter]("documents", buildDocumentColumns("documents"), documentScanMapper, []string{"id"})

// DocumentsQuery is a query on the documents table
type DocumentsQuery = *mysql.ViewQuery[*Document, DocumentSlice]

func buildDocumentColumns(tableName string) documentColumns {
	...
	return documentColumns{
		...
		ID:          buildDocumentColumn(tableName, "id"),
		Body:        buildDocumentColumn(tableName, "body"),
		CreatedAt:   buildDocumentColumn(tableName, "created_at"),
	}
}
...
func (documentColumns) AliasedAs(tableName string) documentColumns {
	return buildDocumentColumns(tableName)
}

// Unqualified returns a copy of the columns set without table qualification.
func (c documentColumns) Unqualified() documentColumns {
	return buildDocumentColumns("")
}

func buildDocumentColumn(alias, name string) documentColumn {
...
func (c documentColumn) Name() string {
	return c.name
}

// ShouldOmitParens prevents automatic parenthesis wrapping in expression builders.
func (c documentColumn) ShouldOmitParens() bool {
	return true
}
...
func documentScanMapper(ctx context.Context, cols []string) (scan.BeforeFunc, func(any) (*Document, error)) {
	// resolve the column positions once per query, not per row
	type target struct {
		idx int
		dst func(o *Document) any
	}
	targets := make([]target, 0, 3)
	for i, col := range cols {
		switch col {
		case "id":
			targets = append(targets, target{i, func(o *Document) any { return &o.ID }})
		case "body":
			targets = append(targets, target{i, func(o *Document) any { return &o.Body }})
		case "created_at":
			targets = append(targets, target{i, func(o *Document) any { return &o.CreatedAt }})
		}
	}
...
func FindDocument(ctx context.Context, exec bob.Executor, IDPK string, cols ...string) (*Document, error) {
	if len(cols) == 0 {
		return Documents.Query(
			sm.Where(Documents.Columns.ID.EQ(mysql.Arg(IDPK))),
		).One(ctx, exec)
	}
...
func (o *Document) pkEQ() dialect.Expression {
	return mysql.Quote("documents", "id").EQ(bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
...
func (o *Document) Reload(ctx context.Context, exec bob.Executor) error {
	o2, err := Documents.Query(
		sm.Where(Documents.Columns.ID.EQ(mysql.Arg(o.ID))),
	).One(ctx, exec)
	if err != nil {
		return err
	}
...
func (o DocumentSlice) pkIN() dialect.Expression {
	...
	return mysql.Quote("documents", "id").In(bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
...
func (o DocumentSlice) copyMatchingRows(from ...*Document) {
	fromByPK := make(map[string]*Document, len(from))
...
func buildDocumentWhere[Q mysql.Filterable](cols documentColumns) documentWhere[Q] {
	return documentWhere[Q]{
		...
		Body:      mysql.Where[Q, string](cols.Body.Expression),
