...
var TeamGithubAccounts = mysql.NewTablex[*TeamGithubAccount, TeamGithubAccountSlice, *TeamGithubAccountSetter]("team_github_accounts", buildTeamGithubAccountColumns("team_github_accounts"), teamGithubAccountScanMapper, []string{"id"})
...
type teamGithubAccountR struct {
	...
	Loaded teamGithubAccountRLoaded `db:"-" `
}

// teamGithubAccountRLoaded tracks which relationships on TeamGithubAccount have been loaded.
type teamGithubAccountRLoaded struct {
	Team bool // team_github_accounts_ibfk_1
}

func buildTeamGithubAccountColumns(tableName string) teamGithubAccountColumns {
	...
	return teamGithubAccountColumns{
		...
		ID:          buildTeamGithubAccountColumn(tableName, "id"),
		TeamID:      buildTeamGithubAccountColumn(tableName, "team_id"),
		GithubID:    buildTeamGithubAccountColumn(tableName, "github_id"),
	}
}
...
type teamGithubAccountColumns struct {
	expr.ColumnsExpr
	tableAlias string
	ID         teamGithubAccountColumn
	TeamID     teamGithubAccountColumn
	GithubID   teamGithubAccountColumn
}
...
func (teamGithubAccountColumns) AliasedAs(tableName string) teamGithubAccountColumns {
	return buildTeamGithubAccountColumns(tableName)
}

// Unqualified returns a copy of the columns set without table qualification.
func (c teamGithubAccountColumns) Unqualified() teamGithubAccountColumns {
	return buildTeamGithubAccountColumns("")
}

func buildTeamGithubAccountColumn(alias, name string) teamGithubAccountColumn {
	return teamGithubAccountColumn{
...
type teamGithubAccountColumn struct {
	mysql.Expression
	alias string
	name  string
}
...
func (c teamGithubAccountColumn) Name() string {
	return c.name
}

// ShouldOmitParens prevents automatic parenthesis wrapping in expression builders.
func (c teamGithubAccountColumn) ShouldOmitParens() bool {
	return true
}
...
func teamGithubAccountScanMapper(ctx context.Context, cols []string) (scan.BeforeFunc, func(any) (*TeamGithubAccount, error)) {
	// resolve the column positions once per query, not per row
	type target struct {
		idx int
		dst func(o *TeamGithubAccount) any
	}
	targets := make([]target, 0, 3)
	for i, col := range cols {
		switch col {
		case "id":
			targets = append(targets, target{i, func(o *TeamGithubAccount) any { return &o.ID }})
		case "team_id":
			targets = append(targets, target{i, func(o *TeamGithubAccount) any { return &o.TeamID }})
		case "github_id":
			targets = append(targets, target{i, func(o *TeamGithubAccount) any { return &o.GithubID }})
		}
	}

	return func(row *scan.Row) (any, error) {
			o := new(TeamGithubAccount)
			for _, t := range targets {
				row.ScheduleScanByIndex(t.idx, t.dst(o))
			}
			return o, nil
		}, func(v any) (*TeamGithubAccount, error) {
			return v.(*TeamGithubAccount), nil
		}
}
...
func (o TeamGithubAccountSlice) copyMatchingRows(from ...*TeamGithubAccount) {
	fromByPK := make(map[string]*TeamGithubAccount, len(from))
...
func (teamGithubAccount0 *TeamGithubAccount) InsertTeam(ctx context.Context, exec bob.Executor, related *TeamSetter) error {
	...
	teamGithubAccount0.R.Loaded.Team = true
...
func buildTeamGithubAccountWhere[Q mysql.Filterable](cols teamGithubAccountColumns) teamGithubAccountWhere[Q] {
	return teamGithubAccountWhere[Q]{
		...
		GithubID: mysql.Where[Q, string](cols.GithubID.Expression),
		R:        teamGithubAccountWhereR[Q]{cols: cols},
	}
}

// teamGithubAccountWhereR holds the relationship-based filters of
// teamGithubAccountWhere under the R namespace — mirroring the model's
// R struct — so they cannot collide with column-based filter fields.
type teamGithubAccountWhereR[Q mysql.Filterable] struct {
	cols teamGithubAccountColumns
}
...
func (w teamGithubAccountWhereR[Q]) HasTeam(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := mysql.Select(
...
func (os TeamGithubAccountSlice) LoadTeam(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	...
	teamGithubAccountByKey := make(map[string][]*TeamGithubAccount, len(os))
	for _, o := range os {
		if o == nil {
			continue
		}

		teamGithubAccountByKey[o.TeamID] = append(teamGithubAccountByKey[o.TeamID], o)
	}

	for _, rel := range teams {

		owners, ok := teamGithubAccountByKey[rel.ID]
		if !ok {
			continue
		}
