// Code generated by BobGen mysql v0.49.0. DO NOT EDIT.
// This file is meant to be re-generated in place and/or deleted at any time.

package models

import (
	"context"
	"database/sql"
	"fmt"
	"io"
	"strings"

	"github.com/stephenafamo/bob"
	"github.com/stephenafamo/bob/dialect/mysql"
	"github.com/stephenafamo/bob/dialect/mysql/dialect"
	"github.com/stephenafamo/bob/dialect/mysql/dm"
	"github.com/stephenafamo/bob/dialect/mysql/sm"
	"github.com/stephenafamo/bob/dialect/mysql/um"
	"github.com/stephenafamo/bob/expr"
	"github.com/stephenafamo/bob/mods"
	"github.com/stephenafamo/bob/orm"
	"github.com/stephenafamo/scan"
)

// BenchmarkLog is an object representing the database table.
type BenchmarkLog struct {
	BenchmarkID string `db:"benchmark_id,pk" `
	UserLog     string `db:"user_log" `
	AdminLog    string `db:"admin_log" `

	R benchmarkLogR `db:"-" `
}

// BenchmarkLogSlice is an alias for a slice of pointers to BenchmarkLog.
// This should almost always be used instead of []*BenchmarkLog.
type BenchmarkLogSlice []*BenchmarkLog

// BenchmarkLogs contains methods to work with the benchmark_logs table
var BenchmarkLogs = mysql.NewTablex[*BenchmarkLog, BenchmarkLogSlice, *BenchmarkLogSetter]("benchmark_logs", buildBenchmarkLogColumns("benchmark_logs"), benchmarkLogScanMapper, []string{"benchmark_id"})

// BenchmarkLogsQuery is a query on the benchmark_logs table
type BenchmarkLogsQuery = *mysql.ViewQuery[*BenchmarkLog, BenchmarkLogSlice]

// benchmarkLogR is where relationships are stored.
type benchmarkLogR struct {
	Benchmark *Benchmark // benchmark_logs_ibfk_1
	// Loaded reports whether each relationship has been loaded.
	// A relationship's bool is set by Load*, Preload, ThenLoad, factory builds,
	// and to-one Attach/Insert operations. To-many Attach/Insert operations leave it unchanged.
	Loaded benchmarkLogRLoaded `db:"-" `
}

// benchmarkLogRLoaded tracks which relationships on BenchmarkLog have been loaded.
type benchmarkLogRLoaded struct {
	Benchmark bool // benchmark_logs_ibfk_1
}

func buildBenchmarkLogColumns(tableName string) benchmarkLogColumns {
	columnsExpr := expr.NewColumnsExpr(
		"benchmark_id", "user_log", "admin_log",
	)

	if tableName != "" {
		columnsExpr = columnsExpr.WithParent(tableName)
	}

	return benchmarkLogColumns{
		ColumnsExpr: columnsExpr,
		tableAlias:  tableName,
		BenchmarkID: buildBenchmarkLogColumn(tableName, "benchmark_id"),
		UserLog:     buildBenchmarkLogColumn(tableName, "user_log"),
		AdminLog:    buildBenchmarkLogColumn(tableName, "admin_log"),
	}
}

type benchmarkLogColumns struct {
	expr.ColumnsExpr
	tableAlias  string
	BenchmarkID benchmarkLogColumn
	UserLog     benchmarkLogColumn
	AdminLog    benchmarkLogColumn
}

// Alias returns the current table alias for the columns set.
func (c benchmarkLogColumns) Alias() string {
	return c.tableAlias
}

// AliasedAs returns a copy of the columns set qualified by tableName.
func (benchmarkLogColumns) AliasedAs(tableName string) benchmarkLogColumns {
	return buildBenchmarkLogColumns(tableName)
}

// Unqualified returns a copy of the columns set without table qualification.
func (c benchmarkLogColumns) Unqualified() benchmarkLogColumns {
	return buildBenchmarkLogColumns("")
}

func buildBenchmarkLogColumn(alias, name string) benchmarkLogColumn {
	return benchmarkLogColumn{
		Expression: mysql.Quote(alias, name),
		alias:      alias,
		name:       name,
	}
}

type benchmarkLogColumn struct {
	mysql.Expression
	alias string
	name  string
}

// Name returns the unqualified column name.
func (c benchmarkLogColumn) Name() string {
	return c.name
}

// ShouldOmitParens prevents automatic parenthesis wrapping in expression builders.
func (c benchmarkLogColumn) ShouldOmitParens() bool {
	return true
}

// BenchmarkLogSetter is used for insert/upsert/update operations
// All values are optional, and do not have to be set
// Generated columns are not included
type BenchmarkLogSetter struct {
	BenchmarkID *string `db:"benchmark_id,pk" `
	UserLog     *string `db:"user_log" `
	AdminLog    *string `db:"admin_log" `
}

func (s BenchmarkLogSetter) SetColumns() []string {
	vals := make([]string, 0, 3)
	if s.BenchmarkID != nil {
		vals = append(vals, "benchmark_id")
	}
	if s.UserLog != nil {
		vals = append(vals, "user_log")
	}
	if s.AdminLog != nil {
		vals = append(vals, "admin_log")
	}
	return vals
}

func (s BenchmarkLogSetter) Overwrite(t *BenchmarkLog) {
	if s.BenchmarkID != nil {
		t.BenchmarkID = func() string {
			if s.BenchmarkID == nil {
				return *new(string)
			}
			return *s.BenchmarkID
		}()
	}
	if s.UserLog != nil {
		t.UserLog = func() string {
			if s.UserLog == nil {
				return *new(string)
			}
			return *s.UserLog
		}()
	}
	if s.AdminLog != nil {
		t.AdminLog = func() string {
			if s.AdminLog == nil {
				return *new(string)
			}
			return *s.AdminLog
		}()
	}
}

func (s *BenchmarkLogSetter) Apply(q *dialect.InsertQuery) {
	q.AppendHooks(func(ctx context.Context, exec bob.Executor) (context.Context, error) {
		return BenchmarkLogs.BeforeInsertHooks.RunHooks(ctx, exec, s)
	})

	q.AppendValues(
		bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
			if s.BenchmarkID == nil {
				return mysql.Raw("DEFAULT").WriteSQL(ctx, w, d, start)
			}
			return mysql.Arg(func() string {
				if s.BenchmarkID == nil {
					return *new(string)
				}
				return *s.BenchmarkID
			}()).WriteSQL(ctx, w, d, start)
		}), bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
			if s.UserLog == nil {
				return mysql.Raw("DEFAULT").WriteSQL(ctx, w, d, start)
			}
			return mysql.Arg(func() string {
				if s.UserLog == nil {
					return *new(string)
				}
				return *s.UserLog
			}()).WriteSQL(ctx, w, d, start)
		}), bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
			if s.AdminLog == nil {
				return mysql.Raw("DEFAULT").WriteSQL(ctx, w, d, start)
			}
			return mysql.Arg(func() string {
				if s.AdminLog == nil {
					return *new(string)
				}
				return *s.AdminLog
			}()).WriteSQL(ctx, w, d, start)
		}))
}

func (s BenchmarkLogSetter) UpdateMod() bob.Mod[*dialect.UpdateQuery] {
	return um.Set(s.Expressions("benchmark_logs")...)
}

func (s BenchmarkLogSetter) Expressions(prefix ...string) []bob.Expression {
	exprs := make([]bob.Expression, 0, 3)

	if s.BenchmarkID != nil {
		exprs = append(exprs, expr.Join{Sep: " = ", Exprs: []bob.Expression{
			mysql.Quote(append(prefix, "benchmark_id")...),
			mysql.Arg(s.BenchmarkID),
		}})
	}

	if s.UserLog != nil {
		exprs = append(exprs, expr.Join{Sep: " = ", Exprs: []bob.Expression{
			mysql.Quote(append(prefix, "user_log")...),
			mysql.Arg(s.UserLog),
		}})
	}

	if s.AdminLog != nil {
		exprs = append(exprs, expr.Join{Sep: " = ", Exprs: []bob.Expression{
			mysql.Quote(append(prefix, "admin_log")...),
			mysql.Arg(s.AdminLog),
		}})
	}

	return exprs
}

// scan.Mapper[*T] without reflection: maps columns to fields by index.
func benchmarkLogScanMapper(ctx context.Context, cols []string) (scan.BeforeFunc, func(any) (*BenchmarkLog, error)) {
	// resolve the column positions once per query, not per row
	type target struct {
		idx int
		dst func(o *BenchmarkLog) any
	}
	targets := make([]target, 0, 3)
	for i, col := range cols {
		switch col {
		case "benchmark_id":
			targets = append(targets, target{i, func(o *BenchmarkLog) any { return &o.BenchmarkID }})
		case "user_log":
			targets = append(targets, target{i, func(o *BenchmarkLog) any { return &o.UserLog }})
		case "admin_log":
			targets = append(targets, target{i, func(o *BenchmarkLog) any { return &o.AdminLog }})
		}
	}

	return func(row *scan.Row) (any, error) {
			o := new(BenchmarkLog)
			for _, t := range targets {
				row.ScheduleScanByIndex(t.idx, t.dst(o))
			}
			return o, nil
		}, func(v any) (*BenchmarkLog, error) {
			return v.(*BenchmarkLog), nil
		}
}

// FindBenchmarkLog retrieves a single record by primary key
// If cols is empty Find will return all columns.
func FindBenchmarkLog(ctx context.Context, exec bob.Executor, BenchmarkIDPK string, cols ...string) (*BenchmarkLog, error) {
	if len(cols) == 0 {
		return BenchmarkLogs.Query(
			sm.Where(BenchmarkLogs.Columns.BenchmarkID.EQ(mysql.Arg(BenchmarkIDPK))),
		).One(ctx, exec)
	}

	return BenchmarkLogs.Query(
		sm.Where(BenchmarkLogs.Columns.BenchmarkID.EQ(mysql.Arg(BenchmarkIDPK))),
		sm.Columns(BenchmarkLogs.Columns.Only(cols...)),
	).One(ctx, exec)
}

// BenchmarkLogExists checks the presence of a single record by primary key
func BenchmarkLogExists(ctx context.Context, exec bob.Executor, BenchmarkIDPK string) (bool, error) {
	return BenchmarkLogs.Query(
		sm.Where(BenchmarkLogs.Columns.BenchmarkID.EQ(mysql.Arg(BenchmarkIDPK))),
	).Exists(ctx, exec)
}

// AfterQueryHook is called after BenchmarkLog is retrieved from the database
func (o *BenchmarkLog) AfterQueryHook(ctx context.Context, exec bob.Executor, queryType bob.QueryType) error {
	var err error

	switch queryType {
	case bob.QueryTypeSelect:
		ctx, err = BenchmarkLogs.AfterSelectHooks.RunHooks(ctx, exec, BenchmarkLogSlice{o})
	case bob.QueryTypeInsert:
		ctx, err = BenchmarkLogs.AfterInsertHooks.RunHooks(ctx, exec, BenchmarkLogSlice{o})
	case bob.QueryTypeUpdate:
		ctx, err = BenchmarkLogs.AfterUpdateHooks.RunHooks(ctx, exec, BenchmarkLogSlice{o})
	case bob.QueryTypeDelete:
		ctx, err = BenchmarkLogs.AfterDeleteHooks.RunHooks(ctx, exec, BenchmarkLogSlice{o})

	}

	return err
}

// primaryKeyVals returns the primary key values of the BenchmarkLog
func (o *BenchmarkLog) primaryKeyVals() bob.Expression {
	return mysql.Arg(o.BenchmarkID)
}

func (o *BenchmarkLog) pkEQ() dialect.Expression {
	return mysql.Quote("benchmark_logs", "benchmark_id").EQ(bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
		return o.primaryKeyVals().WriteSQL(ctx, w, d, start)
	}))
}

// Update uses an executor to update the BenchmarkLog
func (o *BenchmarkLog) Update(ctx context.Context, exec bob.Executor, s *BenchmarkLogSetter) error {
	_, err := BenchmarkLogs.Update(s.UpdateMod(), um.Where(o.pkEQ())).Exec(ctx, exec)
	if err != nil {
		return err
	}

	s.Overwrite(o)

	return nil
}

// Delete deletes a single BenchmarkLog record with an executor
func (o *BenchmarkLog) Delete(ctx context.Context, exec bob.Executor) error {
	_, err := BenchmarkLogs.Delete(dm.Where(o.pkEQ())).Exec(ctx, exec)
	return err
}

// Reload refreshes the BenchmarkLog using the executor
func (o *BenchmarkLog) Reload(ctx context.Context, exec bob.Executor) error {
	o2, err := BenchmarkLogs.Query(
		sm.Where(BenchmarkLogs.Columns.BenchmarkID.EQ(mysql.Arg(o.BenchmarkID))),
	).One(ctx, exec)
	if err != nil {
		return err
	}
	o2.R = o.R
	*o = *o2

	return nil
}

// AfterQueryHook is called after BenchmarkLogSlice is retrieved from the database
func (o BenchmarkLogSlice) AfterQueryHook(ctx context.Context, exec bob.Executor, queryType bob.QueryType) error {
	var err error

	switch queryType {
	case bob.QueryTypeSelect:
		ctx, err = BenchmarkLogs.AfterSelectHooks.RunHooks(ctx, exec, o)
	case bob.QueryTypeInsert:
		ctx, err = BenchmarkLogs.AfterInsertHooks.RunHooks(ctx, exec, o)
	case bob.QueryTypeUpdate:
		ctx, err = BenchmarkLogs.AfterUpdateHooks.RunHooks(ctx, exec, o)
	case bob.QueryTypeDelete:
		ctx, err = BenchmarkLogs.AfterDeleteHooks.RunHooks(ctx, exec, o)

	}

	return err
}

func (o BenchmarkLogSlice) pkIN() dialect.Expression {
	if len(o) == 0 {
		return mysql.Raw("NULL")
	}

	return mysql.Quote("benchmark_logs", "benchmark_id").In(bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
		pkPairs := make([]bob.Expression, len(o))
		for i, row := range o {
			pkPairs[i] = row.primaryKeyVals()
		}
		return bob.ExpressSlice(ctx, w, d, start, pkPairs, "", ", ", "")
	}))
}

// copyMatchingRows finds models in the given slice that have the same primary key
// then it first copies the existing relationships from the old model to the new model
// and then replaces the old model in the slice with the new model
func (o BenchmarkLogSlice) copyMatchingRows(from ...*BenchmarkLog) {
	fromByPK := make(map[string]*BenchmarkLog, len(from))
	for _, new := range from {
		// keep the first row for each key, like the nested loop did
		if _, ok := fromByPK[new.BenchmarkID]; !ok {
			fromByPK[new.BenchmarkID] = new
		}
	}

	for i, old := range o {
		new, ok := fromByPK[old.BenchmarkID]
		if !ok {
			continue
		}
		new.R = old.R
		o[i] = new
	}
}

// UpdateMod modifies an update query with "WHERE primary_key IN (o...)"
func (o BenchmarkLogSlice) UpdateMod() bob.Mod[*dialect.UpdateQuery] {
	return bob.ModFunc[*dialect.UpdateQuery](func(q *dialect.UpdateQuery) {
		q.AppendHooks(func(ctx context.Context, exec bob.Executor) (context.Context, error) {
			return BenchmarkLogs.BeforeUpdateHooks.RunHooks(ctx, exec, o)
		})

		q.AppendLoader(bob.LoaderFunc(func(ctx context.Context, exec bob.Executor, retrieved any) error {
			var err error
			switch retrieved := retrieved.(type) {
			case *BenchmarkLog:
				o.copyMatchingRows(retrieved)
			case []*BenchmarkLog:
				o.copyMatchingRows(retrieved...)
			case BenchmarkLogSlice:
				o.copyMatchingRows(retrieved...)
			default:
				// If the retrieved value is not a BenchmarkLog or a slice of BenchmarkLog
				// then run the AfterUpdateHooks on the slice
				_, err = BenchmarkLogs.AfterUpdateHooks.RunHooks(ctx, exec, o)
			}

			return err
		}))

		q.AppendWhere(o.pkIN())
	})
}

// DeleteMod modifies an delete query with "WHERE primary_key IN (o...)"
func (o BenchmarkLogSlice) DeleteMod() bob.Mod[*dialect.DeleteQuery] {
	return bob.ModFunc[*dialect.DeleteQuery](func(q *dialect.DeleteQuery) {
		q.AppendHooks(func(ctx context.Context, exec bob.Executor) (context.Context, error) {
			return BenchmarkLogs.BeforeDeleteHooks.RunHooks(ctx, exec, o)
		})

		q.AppendLoader(bob.LoaderFunc(func(ctx context.Context, exec bob.Executor, retrieved any) error {
			var err error
			switch retrieved := retrieved.(type) {
			case *BenchmarkLog:
				o.copyMatchingRows(retrieved)
			case []*BenchmarkLog:
				o.copyMatchingRows(retrieved...)
			case BenchmarkLogSlice:
				o.copyMatchingRows(retrieved...)
			default:
				// If the retrieved value is not a BenchmarkLog or a slice of BenchmarkLog
				// then run the AfterDeleteHooks on the slice
				_, err = BenchmarkLogs.AfterDeleteHooks.RunHooks(ctx, exec, o)
			}

			return err
		}))

		q.AppendWhere(o.pkIN())
	})
}

func (o BenchmarkLogSlice) UpdateAll(ctx context.Context, exec bob.Executor, vals BenchmarkLogSetter) error {
	_, err := BenchmarkLogs.Update(vals.UpdateMod(), o.UpdateMod()).Exec(ctx, exec)

	for i := range o {
		vals.Overwrite(o[i])
	}

	return err
}

func (o BenchmarkLogSlice) DeleteAll(ctx context.Context, exec bob.Executor) error {
	if len(o) == 0 {
		return nil
	}

	_, err := BenchmarkLogs.Delete(o.DeleteMod()).Exec(ctx, exec)
	return err
}

func (o BenchmarkLogSlice) ReloadAll(ctx context.Context, exec bob.Executor) error {
	if len(o) == 0 {
		return nil
	}

	o2, err := BenchmarkLogs.Query(sm.Where(o.pkIN())).All(ctx, exec)
	if err != nil {
		return err
	}

	o.copyMatchingRows(o2...)

	return nil
}

// Benchmark starts a query for related objects on benchmarks
func (o *BenchmarkLog) Benchmark(mods ...bob.Mod[*dialect.SelectQuery]) BenchmarksQuery {
	return Benchmarks.Query(append(mods,
		sm.Where(Benchmarks.Columns.ID.EQ(mysql.Arg(o.BenchmarkID))),
	)...)
}

func (os BenchmarkLogSlice) Benchmark(mods ...bob.Mod[*dialect.SelectQuery]) BenchmarksQuery {
	PKArgSlice := make([]bob.Expression, len(os))
	for i, o := range os {
		PKArgSlice[i] = mysql.ArgGroup(o.BenchmarkID)
	}
	PKArgExpr := mysql.Group(PKArgSlice...)

	return Benchmarks.Query(append(mods,
		sm.Where(mysql.Group(Benchmarks.Columns.ID).OP("IN", PKArgExpr)),
	)...)
}

func attachBenchmarkLogBenchmark0(ctx context.Context, exec bob.Executor, count int, benchmarkLog0 *BenchmarkLog, benchmark1 *Benchmark) (*BenchmarkLog, error) {
	setter := &BenchmarkLogSetter{
		BenchmarkID: func() *string { return &benchmark1.ID }(),
	}

	err := benchmarkLog0.Update(ctx, exec, setter)
	if err != nil {
		return nil, fmt.Errorf("attachBenchmarkLogBenchmark0: %w", err)
	}

	return benchmarkLog0, nil
}

func (benchmarkLog0 *BenchmarkLog) InsertBenchmark(ctx context.Context, exec bob.Executor, related *BenchmarkSetter) error {
	var err error

	benchmark1, err := Benchmarks.Insert(related).One(ctx, exec)
	if err != nil {
		return fmt.Errorf("inserting related objects: %w", err)
	}

	_, err = attachBenchmarkLogBenchmark0(ctx, exec, 1, benchmarkLog0, benchmark1)
	if err != nil {
		return err
	}

	benchmarkLog0.R.Benchmark = benchmark1
	benchmarkLog0.R.Loaded.Benchmark = true

	benchmark1.R.BenchmarkLog = benchmarkLog0
	benchmark1.R.Loaded.BenchmarkLog = true

	return nil
}

func (benchmarkLog0 *BenchmarkLog) AttachBenchmark(ctx context.Context, exec bob.Executor, benchmark1 *Benchmark) error {
	var err error

	_, err = attachBenchmarkLogBenchmark0(ctx, exec, 1, benchmarkLog0, benchmark1)
	if err != nil {
		return err
	}

	benchmarkLog0.R.Benchmark = benchmark1
	benchmarkLog0.R.Loaded.Benchmark = true

	benchmark1.R.BenchmarkLog = benchmarkLog0
	benchmark1.R.Loaded.BenchmarkLog = true

	return nil
}

type benchmarkLogWhere[Q mysql.Filterable] struct {
	cols        benchmarkLogColumns
	BenchmarkID mysql.WhereMod[Q, string]
	UserLog     mysql.WhereMod[Q, string]
	AdminLog    mysql.WhereMod[Q, string]
	R           benchmarkLogWhereR[Q]
}

func (benchmarkLogWhere[Q]) AliasedAs(alias string) benchmarkLogWhere[Q] {
	return buildBenchmarkLogWhere[Q](buildBenchmarkLogColumns(alias))
}

func buildBenchmarkLogWhere[Q mysql.Filterable](cols benchmarkLogColumns) benchmarkLogWhere[Q] {
	return benchmarkLogWhere[Q]{
		cols:        cols,
		BenchmarkID: mysql.Where[Q, string](cols.BenchmarkID.Expression),
		UserLog:     mysql.Where[Q, string](cols.UserLog.Expression),
		AdminLog:    mysql.Where[Q, string](cols.AdminLog.Expression),
		R:           benchmarkLogWhereR[Q]{cols: cols},
	}
}

// benchmarkLogWhereR holds the relationship-based filters of
// benchmarkLogWhere under the R namespace — mirroring the model's
// R struct — so they cannot collide with column-based filter fields.
type benchmarkLogWhereR[Q mysql.Filterable] struct {
	cols benchmarkLogColumns
}

// HasBenchmark filters parents that have a matching Benchmark using a
// correlated EXISTS subquery (semi-join). Unlike an INNER JOIN it does not
// multiply parent rows, so no DISTINCT is needed. The optional filters are
// applied to the subquery (i.e. to Benchmarks).
func (w benchmarkLogWhereR[Q]) HasBenchmark(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := mysql.Select(
		sm.Columns(mysql.Raw("1")),
		sm.From(Benchmarks.NameExpr()),
		sm.Where(Benchmarks.Columns.ID.EQ(w.cols.BenchmarkID)),
	)
	q.Apply(filters...)
	return mods.Where[Q]{E: mysql.Exists(q)}
}

// NULL-tolerant scan buffer for benchmarkLogScanMapperNullable:
// on a LEFT JOIN miss every column comes back NULL, so each field uses the
// nullable version of the column type even when the column itself is NOT NULL.
type benchmarkLogPreloadBuf struct {
	BenchmarkID sql.Null[string]
	UserLog     sql.Null[string]
	AdminLog    sql.Null[string]
}

// benchmarkLogScanMapperNullable maps the preloaded benchmarkLog
// columns (prefixed with the runtime join alias) without reflection, while
// keeping the LEFT JOIN semantics of the reflection-based mapper: a row whose
// prefixed columns are all NULL yields nil (no child), and NULL values never
// error, they just leave the zero value in the field.
func benchmarkLogScanMapperNullable(prefix string) scan.Mapper[*BenchmarkLog] {
	return func(ctx context.Context, cols []string) (scan.BeforeFunc, func(any) (*BenchmarkLog, error)) {
		// resolve the column names once per query, not per row
		type target struct {
			idx int
			dst func(b *benchmarkLogPreloadBuf) any
		}
		targets := make([]target, 0, 3)
		for i, col := range cols {
			name, ok := strings.CutPrefix(col, prefix)
			if !ok {
				continue
			}
			switch name {
			case "benchmark_id":
				targets = append(targets, target{i, func(b *benchmarkLogPreloadBuf) any { return &b.BenchmarkID }})
			case "user_log":
				targets = append(targets, target{i, func(b *benchmarkLogPreloadBuf) any { return &b.UserLog }})
			case "admin_log":
				targets = append(targets, target{i, func(b *benchmarkLogPreloadBuf) any { return &b.AdminLog }})
			}
		}

		// One scratch buffer reused for every row: before -> scan -> after runs
		// synchronously per row, the scheduled columns are fully overwritten each
		// row, and unselected columns stay zero, so a single buffer preserves the
		// per-row NULL semantics without allocating a buffer per parent row. The
		// returned child (o below) is always freshly allocated, so buf never
		// escapes past the after func.
		buf := new(benchmarkLogPreloadBuf)
		return func(row *scan.Row) (any, error) {
				for _, t := range targets {
					row.ScheduleScanByIndex(t.idx, t.dst(buf))
				}
				return buf, nil
			}, func(link any) (*BenchmarkLog, error) {
				buf := link.(*benchmarkLogPreloadBuf)

				// Same rule as the reflection mapper's row validator: the child
				// exists only if at least one of its columns is not NULL. Columns
				// not selected by the query are never scanned and stay invalid,
				// so this check also matches when only a subset is selected.
				if !(buf.BenchmarkID.Valid) &&
					!(buf.UserLog.Valid) &&
					!(buf.AdminLog.Valid) {
					return nil, nil
				}

				o := new(BenchmarkLog)
				if buf.BenchmarkID.Valid {
					o.BenchmarkID = buf.BenchmarkID.V
				}
				if buf.UserLog.Valid {
					o.UserLog = buf.UserLog.V
				}
				if buf.AdminLog.Valid {
					o.AdminLog = buf.AdminLog.V
				}
				return o, nil
			}
	}
}

func (o *BenchmarkLog) Preload(name string, retrieved any) error {
	if o == nil {
		return nil
	}

	switch name {
	case "Benchmark":
		rel, ok := retrieved.(*Benchmark)
		if !ok {
			return fmt.Errorf("benchmarkLog cannot load %T as %q", retrieved, name)
		}

		o.R.Benchmark = rel
		o.R.Loaded.Benchmark = true

		if rel != nil {
			rel.R.BenchmarkLog = o
			rel.R.Loaded.BenchmarkLog = true
		}
		return nil
	default:
		return fmt.Errorf("benchmarkLog has no relationship %q", name)
	}
}

type benchmarkLogPreloader struct {
	Benchmark func(...mysql.PreloadOption) mysql.Preloader
}

func buildBenchmarkLogPreloader() benchmarkLogPreloader {
	return benchmarkLogPreloader{
		Benchmark: func(opts ...mysql.PreloadOption) mysql.Preloader {
			return mysql.Preload[*Benchmark, BenchmarkSlice](mysql.PreloadRel{
				Name: "Benchmark",
				Sides: []mysql.PreloadSide{
					{
						From:        BenchmarkLogs,
						To:          Benchmarks,
						FromColumns: []string{"benchmark_id"},
						ToColumns:   []string{"id"},
					},
				},
			}, Benchmarks.Columns.Names(), benchmarkScanMapperNullable, opts...)
		},
	}
}

type benchmarkLogThenLoader[Q orm.Loadable] struct {
	Benchmark func(...bob.Mod[*dialect.SelectQuery]) orm.Loader[Q]
}

func buildBenchmarkLogThenLoader[Q orm.Loadable]() benchmarkLogThenLoader[Q] {
	type BenchmarkLoadInterface interface {
		LoadBenchmark(context.Context, bob.Executor, ...bob.Mod[*dialect.SelectQuery]) error
	}

	return benchmarkLogThenLoader[Q]{
		Benchmark: thenLoadBuilder[Q](
			"Benchmark",
			func(ctx context.Context, exec bob.Executor, retrieved BenchmarkLoadInterface, mods ...bob.Mod[*dialect.SelectQuery]) error {
				return retrieved.LoadBenchmark(ctx, exec, mods...)
			},
		),
	}
}

// LoadBenchmark loads the benchmarkLog's Benchmark into the .R struct
func (o *BenchmarkLog) LoadBenchmark(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if o == nil {
		return nil
	}

	// Reset the relationship
	o.R.Benchmark = nil
	o.R.Loaded.Benchmark = false

	related, err := o.Benchmark(mods...).One(ctx, exec)
	if err != nil {
		return err
	}

	related.R.BenchmarkLog = o
	related.R.Loaded.BenchmarkLog = true

	o.R.Benchmark = related
	o.R.Loaded.Benchmark = true
	return nil
}

// LoadBenchmark loads the benchmarkLog's Benchmark into the .R struct
func (os BenchmarkLogSlice) LoadBenchmark(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if len(os) == 0 {
		return nil
	}

	benchmarks, err := os.Benchmark(mods...).All(ctx, exec)
	if err != nil {
		return err
	}

	for _, o := range os {
		if o == nil {
			continue
		}

		o.R.Benchmark = nil
		o.R.Loaded.Benchmark = true
	}
	// O(N+M) stitch via a map keyed by the join column (key -> []parent; was O(N*M)).
	benchmarkLogByKey := make(map[string][]*BenchmarkLog, len(os))
	for _, o := range os {
		if o == nil {
			continue
		}

		benchmarkLogByKey[o.BenchmarkID] = append(benchmarkLogByKey[o.BenchmarkID], o)
	}

	for _, rel := range benchmarks {

		owners, ok := benchmarkLogByKey[rel.ID]
		if !ok {
			continue
		}

		for _, o := range owners {

			// to-one: keep only the first matching child (matches the previous break)
			if o.R.Benchmark != nil {
				continue
			}

			rel.R.BenchmarkLog = o
			rel.R.Loaded.BenchmarkLog = true

			o.R.Benchmark = rel

		}
	}

	return nil
}

type benchmarkLogJoins[Q dialect.Joinable] struct {
	typ       string
	Benchmark modAs[Q, benchmarkColumns]
}

func (j benchmarkLogJoins[Q]) aliasedAs(alias string) benchmarkLogJoins[Q] {
	return buildBenchmarkLogJoins[Q](buildBenchmarkLogColumns(alias), j.typ)
}

func buildBenchmarkLogJoins[Q dialect.Joinable](cols benchmarkLogColumns, typ string) benchmarkLogJoins[Q] {
	return benchmarkLogJoins[Q]{
		typ: typ,
		Benchmark: modAs[Q, benchmarkColumns]{
			c: Benchmarks.Columns,
			f: func(to benchmarkColumns) bob.Mod[Q] {
				mods := make(mods.QueryMods[Q], 0, 1)

				{
					mods = append(mods, dialect.Join[Q](typ, Benchmarks.NameExpr().As(to.Alias())).On(
						to.ID.EQ(cols.BenchmarkID),
					))
				}

				return mods
			},
		},
	}
}
