...
func buildTeamJoins[Q dialect.Joinable](cols teamColumns, typ string) teamJoins[Q] {
	return teamJoins[Q]{
		typ:   typ,
		Users: teamsJoinUsers[Q](cols, typ),
	}
}

func teamsJoinUsers[Q dialect.Joinable](from teamColumns, typ string) func(context.Context) modAs[Q, userColumns] {
...
func (os TeamSlice) Users(mods ...bob.Mod[*dialect.SelectQuery]) UsersQuery {
	...
	return Users.Query(append(mods,
		sm.Where(mysql.Group(UserColumns.TeamID).In(PKArgs...)),
	)...)
}
...
func ThenLoadTeamUsers(queryMods ...bob.Mod[*dialect.SelectQuery]) mysql.Loader {
	return mysql.Loader(func(ctx context.Context, exec bob.Executor, retrieved any) error {
		loader, isLoader := retrieved.(interface {
			LoadTeamUsers(context.Context, bob.Executor, ...bob.Mod[*dialect.SelectQuery]) error
		})
		if !isLoader {
			return fmt.Errorf("object %T cannot load TeamUsers", retrieved)
		}

		err := loader.LoadTeamUsers(ctx, exec, queryMods...)
...
// LoadTeamUsers loads the team's Users into the .R struct
func (o *Team) LoadTeamUsers(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if o == nil {
		return nil
	}
...
// LoadTeamUsers loads the team's Users into the .R struct
func (os TeamSlice) LoadTeamUsers(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if len(os) == 0 {
		return nil
	}
...
func insertTeamUsers0(ctx context.Context, exec bob.Executor, users1 []*UserSetter, team0 *Team) (UserSlice, error) {
	for i := range users1 {
		users1[i].TeamID = omitnull.From(team0.ID)
	}
