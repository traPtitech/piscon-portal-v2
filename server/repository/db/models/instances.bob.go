...
type Instance struct {
	...
	PublicIP       null.Val[string] `db:"public_ip" `
	PrivateIP      null.Val[string] `db:"private_ip" `
...
var Instances = mysql.NewTablex[*Instance, InstanceSlice, *InstanceSetter]("instances", []string{"id"})
...
type instanceColumnNames struct {
	ID             string
	TeamID         string
...
type InstanceSetter struct {
	...
	InstanceNumber omit.Val[int32]           `db:"instance_number" `
	Status         omit.Val[InstancesStatus] `db:"status" `
	CreatedAt      omit.Val[time.Time]       `db:"created_at" `
	PublicIP       omitnull.Val[string]      `db:"public_ip" `
	PrivateIP      omitnull.Val[string]      `db:"private_ip" `
}
...
func (s InstanceSetter) SetColumns() []string {
	...
	if !s.PublicIP.IsUnset() {
		vals = append(vals, "public_ip")
	}

	if !s.PrivateIP.IsUnset() {
...
func (s InstanceSetter) Overwrite(t *Instance) {
	...
	if !s.InstanceNumber.IsUnset() {
		t.InstanceNumber, _ = s.InstanceNumber.Get()
	}
	if !s.Status.IsUnset() {
	...
	if !s.PublicIP.IsUnset() {
		t.PublicIP, _ = s.PublicIP.GetNull()
	}
	if !s.PrivateIP.IsUnset() {
		t.PrivateIP, _ = s.PrivateIP.GetNull()
	}
}
...
func (s *InstanceSetter) Apply(q *dialect.InsertQuery) {
	...
	q.AppendValues(bob.ExpressionFunc(func(ctx context.Context, w io.Writer, d bob.Dialect, start int) ([]any, error) {
		...
		if s.InstanceNumber.IsUnset() {
			...
			vals[2] = mysql.Arg(s.InstanceNumber)
		}
		...
		if s.PublicIP.IsUnset() {
			...
			vals[5] = mysql.Arg(s.PublicIP)
		}

		if s.PrivateIP.IsUnset() {
			vals[6] = mysql.Raw("DEFAULT")
		} else {
			vals[6] = mysql.Arg(s.PrivateIP)
		}
...
func FindInstance(ctx context.Context, exec bob.Executor, IDPK string, cols ...string) (*Instance, error) {
	if len(cols) == 0 {
		return Instances.Query(
			SelectWhere.Instances.ID.EQ(IDPK),
		).One(ctx, exec)
	}
...
func (o *Instance) Reload(ctx context.Context, exec bob.Executor) error {
	o2, err := Instances.Query(
		SelectWhere.Instances.ID.EQ(o.ID),
	).One(ctx, exec)
	if err != nil {
		return err
	}
...
type instanceJoins[Q dialect.Joinable] struct {
	typ        string
	Benchmarks func(context.Context) modAs[Q, benchmarkColumns]
}
...
func buildInstanceJoins[Q dialect.Joinable](cols instanceColumns, typ string) instanceJoins[Q] {
	return instanceJoins[Q]{
		typ:        typ,
		Benchmarks: instancesJoinBenchmarks[Q](cols, typ),
	}
}

func instancesJoinBenchmarks[Q dialect.Joinable](from instanceColumns, typ string) func(context.Context) modAs[Q, benchmarkColumns] {
...
func (o *Instance) Benchmarks(mods ...bob.Mod[*dialect.SelectQuery]) BenchmarksQuery {
	return Benchmarks.Query(append(mods,
		sm.Where(BenchmarkColumns.InstanceID.EQ(mysql.Arg(o.ID))),
	)...)
}
...
func (os InstanceSlice) Benchmarks(mods ...bob.Mod[*dialect.SelectQuery]) BenchmarksQuery {
	...
	return Benchmarks.Query(append(mods,
		sm.Where(mysql.Group(BenchmarkColumns.InstanceID).In(PKArgs...)),
	)...)
}
...
func ThenLoadInstanceBenchmarks(queryMods ...bob.Mod[*dialect.SelectQuery]) mysql.Loader {
	return mysql.Loader(func(ctx context.Context, exec bob.Executor, retrieved any) error {
		loader, isLoader := retrieved.(interface {
			LoadInstanceBenchmarks(context.Context, bob.Executor, ...bob.Mod[*dialect.SelectQuery]) error
		})
		if !isLoader {
			return fmt.Errorf("object %T cannot load InstanceBenchmarks", retrieved)
		}

		err := loader.LoadInstanceBenchmarks(ctx, exec, queryMods...)
...
// LoadInstanceBenchmarks loads the instance's Benchmarks into the .R struct
func (o *Instance) LoadInstanceBenchmarks(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if o == nil {
		return nil
	}
...
// LoadInstanceBenchmarks loads the instance's Benchmarks into the .R struct
func (os InstanceSlice) LoadInstanceBenchmarks(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	if len(os) == 0 {
		return nil
	}
...
func insertInstanceBenchmarks0(ctx context.Context, exec bob.Executor, benchmarks1 []*BenchmarkSetter, instance0 *Instance) (BenchmarkSlice, error) {
	for i := range benchmarks1 {
		benchmarks1[i].InstanceID = omit.From(instance0.ID)
	}
...
func attachInstanceBenchmarks0(ctx context.Context, exec bob.Executor, count int, benchmarks1 BenchmarkSlice, instance0 *Instance) (BenchmarkSlice, error) {
	setter := &BenchmarkSetter{
		InstanceID: omit.From(instance0.ID),
	}
