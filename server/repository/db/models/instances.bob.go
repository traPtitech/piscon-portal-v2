...
var Instances = mysql.NewTablex[*Instance, InstanceSlice, *InstanceSetter]("instances", buildInstanceColumns("instances"), instanceScanMapper, []string{"id"})
...
func instanceScanMapper(ctx context.Context, cols []string) (scan.BeforeFunc, func(any) (*Instance, error)) {
	// resolve the column positions once per query, not per row
	type target struct {
		idx int
		dst func(o *Instance) any
	}
	targets := make([]target, 0, 6)
	for i, col := range cols {
		switch col {
		case "id":
			targets = append(targets, target{i, func(o *Instance) any { return &o.ID }})
		case "provider_instance_id":
			targets = append(targets, target{i, func(o *Instance) any { return &o.ProviderInstanceID }})
		case "team_id":
			targets = append(targets, target{i, func(o *Instance) any { return &o.TeamID }})
		case "instance_number":
			targets = append(targets, target{i, func(o *Instance) any { return &o.InstanceNumber }})
		case "created_at":
			targets = append(targets, target{i, func(o *Instance) any { return &o.CreatedAt }})
		case "deleted_at":
			targets = append(targets, target{i, func(o *Instance) any { return &o.DeletedAt }})
		}
	}
...
func (o InstanceSlice) copyMatchingRows(from ...*Instance) {
	fromByPK := make(map[string]*Instance, len(from))
...
func buildInstanceWhere[Q mysql.Filterable](cols instanceColumns) instanceWhere[Q] {
	return instanceWhere[Q]{
		...
		R:                  instanceWhereR[Q]{cols: cols},
	}
}

// instanceWhereR holds the relationship-based filters of
// instanceWhere under the R namespace — mirroring the model's
// R struct — so they cannot collide with column-based filter fields.
type instanceWhereR[Q mysql.Filterable] struct {
	cols instanceColumns
}

// HasBenchmarks filters parents that have a matching Benchmarks using a
...
// applied to the subquery (i.e. to Benchmarks).
func (w instanceWhereR[Q]) HasBenchmarks(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := mysql.Select(
		sm.Columns(mysql.Raw("1")),
		sm.From(Benchmarks.NameExpr()),
		sm.Where(Benchmarks.Columns.InstanceID.EQ(w.cols.ID)),
	)
	q.Apply(filters...)
	return mods.Where[Q]{E: mysql.Exists(q)}
}

// NULL-tolerant scan buffer for instanceScanMapperNullable:
...
type instancePreloadBuf struct {
	ID                 sql.Null[string]
	ProviderInstanceID sql.Null[string]
	TeamID             sql.Null[string]
	InstanceNumber     sql.Null[int32]
	CreatedAt          sql.Null[time.Time]
	DeletedAt          sql.Null[time.Time]
}

// instanceScanMapperNullable maps the preloaded instance
...
func instanceScanMapperNullable(prefix string) scan.Mapper[*Instance] {
	return func(ctx context.Context, cols []string) (scan.BeforeFunc, func(any) (*Instance, error)) {
		// resolve the column names once per query, not per row
		type target struct {
			idx int
			dst func(b *instancePreloadBuf) any
		}
		targets := make([]target, 0, 6)
		for i, col := range cols {
			...
			switch name {
			case "id":
				targets = append(targets, target{i, func(b *instancePreloadBuf) any { return &b.ID }})
			case "provider_instance_id":
				targets = append(targets, target{i, func(b *instancePreloadBuf) any { return &b.ProviderInstanceID }})
			case "team_id":
				targets = append(targets, target{i, func(b *instancePreloadBuf) any { return &b.TeamID }})
			case "instance_number":
				targets = append(targets, target{i, func(b *instancePreloadBuf) any { return &b.InstanceNumber }})
			case "created_at":
				targets = append(targets, target{i, func(b *instancePreloadBuf) any { return &b.CreatedAt }})
			case "deleted_at":
				targets = append(targets, target{i, func(b *instancePreloadBuf) any { return &b.DeletedAt }})
			}
		}
		...
		buf := new(instancePreloadBuf)
		return func(row *scan.Row) (any, error) {
				for _, t := range targets {
					row.ScheduleScanByIndex(t.idx, t.dst(buf))
				}
				return buf, nil
			}, func(link any) (*Instance, error) {
				buf := link.(*instancePreloadBuf)
				...
				if !(buf.ID.Valid) &&
					!(buf.ProviderInstanceID.Valid) &&
					!(buf.TeamID.Valid) &&
					!(buf.InstanceNumber.Valid) &&
					!(buf.CreatedAt.Valid) &&
					!(buf.DeletedAt.Valid) {
					return nil, nil
				}
				...
				if buf.ProviderInstanceID.Valid {
					o.ProviderInstanceID = buf.ProviderInstanceID.V
				}
				if buf.TeamID.Valid {
					o.TeamID = buf.TeamID.V
				}
				if buf.InstanceNumber.Valid {
					o.InstanceNumber = buf.InstanceNumber.V
				}
...
func (os InstanceSlice) LoadBenchmarks(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	...
	instanceByKey := make(map[string][]*Instance, len(os))
	for _, o := range os {
		if o == nil {
			continue
		}

		instanceByKey[o.ID] = append(instanceByKey[o.ID], o)
	}

	for _, rel := range benchmarks {

		owners, ok := instanceByKey[rel.InstanceID]
		if !ok {
			continue
		}
	...
	return instanceJoins[Q]{
		...
		Benchmarks: modAs[Q, benchmarkColumns]{
			...
			f: func(to benchmarkColumns) bob.Mod[Q] {
				...
				{
					mods = append(mods, dialect.Join[Q](typ, Benchmarks.NameExpr().As(to.Alias())).On(
