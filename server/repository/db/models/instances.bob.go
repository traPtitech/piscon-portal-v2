...
type InstanceSetter struct {
	...
	ProviderInstanceID omit.Val[string]        `db:"provider_instance_id" `
...
func (s InstanceSetter) Overwrite(t *Instance) {
	...
	if s.ProviderInstanceID.IsValue() {
		t.ProviderInstanceID = s.ProviderInstanceID.MustGet()
	}
	if s.TeamID.IsValue() {
		t.TeamID = s.TeamID.MustGet()
	}
	if s.InstanceNumber.IsValue() {
		t.InstanceNumber = s.InstanceNumber.MustGet()
	}
	if s.CreatedAt.IsValue() {
...
func (s *InstanceSetter) Apply(q *dialect.InsertQuery) {
	...
	q.AppendValues(
		bob.ExpressionFunc(func(ctx context.Context, w io.Writer, d bob.Dialect, start int) ([]any, error) {
			...
			return mysql.Arg(s.ProviderInstanceID.MustGet()).WriteSQL(ctx, w, d, start)
		}), bob.ExpressionFunc(func(ctx context.Context, w io.Writer, d bob.Dialect, start int) ([]any, error) {
			if !s.TeamID.IsValue() {
				return mysql.Raw("DEFAULT").WriteSQL(ctx, w, d, start)
			}
			...
			return mysql.Arg(s.InstanceNumber.MustGet()).WriteSQL(ctx, w, d, start)
		}), bob.ExpressionFunc(func(ctx context.Context, w io.Writer, d bob.Dialect, start int) ([]any, error) {
			if !s.CreatedAt.IsValue() {
				return mysql.Raw("DEFAULT").WriteSQL(ctx, w, d, start)
			}
...
func (os InstanceSlice) Benchmarks(mods ...bob.Mod[*dialect.SelectQuery]) BenchmarksQuery {
	...
	return Benchmarks.Query(append(mods,
		sm.Where(mysql.Group(BenchmarkColumns.InstanceID).OP("IN", PKArgExpr)),
	)...)
}
