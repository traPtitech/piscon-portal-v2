...
var Users = mysql.NewTablex[*User, UserSlice, *UserSetter]("users", buildUserColumns("users"), []string{"id"}, []string{"name"})
...
func (os UserSlice) LoadTeam(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	...
	for _, o := range os {
		...
		for _, rel := range teams {
			...
			if !(o.TeamID.Valid && o.TeamID.V == rel.ID) {
				continue
			}
