...
var Sessions = mysql.NewTablex[*Session, SessionSlice, *SessionSetter]("sessions", buildSessionColumns("sessions"), sessionScanMapper, []string{"id"})
...
func buildSessionColumns(tableName string) sessionColumns {
	...
	return sessionColumns{
		...
		ExpiredAt:   buildSessionColumn(tableName, "expired_at"),
	}
}
...
func sessionScanMapper(ctx context.Context, cols []string) (scan.BeforeFunc, func(any) (*Session, error)) {
	...
	for i, col := range cols {
		switch col {
		case "id":
			targets = append(targets, target{i, func(o *Session) any { return &o.ID }})
		case "user_id":
			targets = append(targets, target{i, func(o *Session) any { return &o.UserID }})
		case "created_at":
			targets = append(targets, target{i, func(o *Session) any { return &o.CreatedAt }})
		case "expired_at":
			targets = append(targets, target{i, func(o *Session) any { return &o.ExpiredAt }})
		}
	}
...
func (o *Session) pkEQ() dialect.Expression {
	return mysql.Quote("sessions", "id").EQ(bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
...
func (o SessionSlice) pkIN() dialect.Expression {
	...
	return mysql.Quote("sessions", "id").In(bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
...
func (o SessionSlice) copyMatchingRows(from ...*Session) {
	fromByPK := make(map[string]*Session, len(from))
...
func buildSessionWhere[Q mysql.Filterable](cols sessionColumns) sessionWhere[Q] {
	return sessionWhere[Q]{
		...
		ExpiredAt: mysql.Where[Q, time.Time](cols.ExpiredAt.Expression),
		R:         sessionWhereR[Q]{cols: cols},
	}
}

// sessionWhereR holds the relationship-based filters of
// sessionWhere under the R namespace — mirroring the model's
// R struct — so they cannot collide with column-based filter fields.
type sessionWhereR[Q mysql.Filterable] struct {
	cols sessionColumns
}
...
func (w sessionWhereR[Q]) HasUser(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := mysql.Select(
...
func (os SessionSlice) LoadUser(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	...
	sessionByKey := make(map[string][]*Session, len(os))
	for _, o := range os {
		if o == nil {
			continue
		}

		sessionByKey[o.UserID] = append(sessionByKey[o.UserID], o)
	}

	for _, rel := range users {

		owners, ok := sessionByKey[rel.ID]
		if !ok {
			continue
		}
