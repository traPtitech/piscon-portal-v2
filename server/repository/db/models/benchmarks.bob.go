...
var Benchmarks = mysql.NewTablex[*Benchmark, BenchmarkSlice, *BenchmarkSetter]("benchmarks", buildBenchmarkColumns("benchmarks"), benchmarkScanMapper, []string{"id"})
...
type benchmarkR struct {
	...
	Loaded benchmarkRLoaded `db:"-" `
}

// benchmarkRLoaded tracks which relationships on Benchmark have been loaded.
type benchmarkRLoaded struct {
	BenchmarkLog bool // benchmark_logs_ibfk_1
	Instance     bool // benchmarks_ibfk_1
}

func buildBenchmarkColumns(tableName string) benchmarkColumns {
	...
	return benchmarkColumns{
		...
		ID:           buildBenchmarkColumn(tableName, "id"),
		InstanceID:   buildBenchmarkColumn(tableName, "instance_id"),
		TeamID:       buildBenchmarkColumn(tableName, "team_id"),
		UserID:       buildBenchmarkColumn(tableName, "user_id"),
		Status:       buildBenchmarkColumn(tableName, "status"),
		CreatedAt:    buildBenchmarkColumn(tableName, "created_at"),
		StartedAt:    buildBenchmarkColumn(tableName, "started_at"),
		FinishedAt:   buildBenchmarkColumn(tableName, "finished_at"),
		Score:        buildBenchmarkColumn(tableName, "score"),
		Result:       buildBenchmarkColumn(tableName, "result"),
		ErrorMessage: buildBenchmarkColumn(tableName, "error_message"),
	}
}
...
type benchmarkColumns struct {
	...
	tableAlias   string
	ID           benchmarkColumn
	InstanceID   benchmarkColumn
	TeamID       benchmarkColumn
	UserID       benchmarkColumn
	Status       benchmarkColumn
	CreatedAt    benchmarkColumn
	StartedAt    benchmarkColumn
	FinishedAt   benchmarkColumn
	Score        benchmarkColumn
	Result       benchmarkColumn
	ErrorMessage benchmarkColumn
}
...
func (benchmarkColumns) AliasedAs(tableName string) benchmarkColumns {
	return buildBenchmarkColumns(tableName)
}

// Unqualified returns a copy of the columns set without table qualification.
func (c benchmarkColumns) Unqualified() benchmarkColumns {
	return buildBenchmarkColumns("")
}

func buildBenchmarkColumn(alias, name string) benchmarkColumn {
...
func (c benchmarkColumn) Name() string {
	return c.name
}

// ShouldOmitParens prevents automatic parenthesis wrapping in expression builders.
func (c benchmarkColumn) ShouldOmitParens() bool {
	return true
}
...
func benchmarkScanMapper(ctx context.Context, cols []string) (scan.BeforeFunc, func(any) (*Benchmark, error)) {
	// resolve the column positions once per query, not per row
	type target struct {
		idx int
		dst func(o *Benchmark) any
	}
	targets := make([]target, 0, 11)
	for i, col := range cols {
		switch col {
		case "id":
			targets = append(targets, target{i, func(o *Benchmark) any { return &o.ID }})
		case "instance_id":
			targets = append(targets, target{i, func(o *Benchmark) any { return &o.InstanceID }})
		case "team_id":
			targets = append(targets, target{i, func(o *Benchmark) any { return &o.TeamID }})
		case "user_id":
			targets = append(targets, target{i, func(o *Benchmark) any { return &o.UserID }})
		case "status":
			targets = append(targets, target{i, func(o *Benchmark) any { return &o.Status }})
		case "created_at":
			targets = append(targets, target{i, func(o *Benchmark) any { return &o.CreatedAt }})
		case "started_at":
			targets = append(targets, target{i, func(o *Benchmark) any { return &o.StartedAt }})
		case "finished_at":
			targets = append(targets, target{i, func(o *Benchmark) any { return &o.FinishedAt }})
		case "score":
			targets = append(targets, target{i, func(o *Benchmark) any { return &o.Score }})
		case "result":
			targets = append(targets, target{i, func(o *Benchmark) any { return &o.Result }})
		case "error_message":
			targets = append(targets, target{i, func(o *Benchmark) any { return &o.ErrorMessage }})
		}
	}
...
func (o *Benchmark) pkEQ() dialect.Expression {
	return mysql.Quote("benchmarks", "id").EQ(bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
...
func (o BenchmarkSlice) pkIN() dialect.Expression {
	...
	return mysql.Quote("benchmarks", "id").In(bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
...
func (o BenchmarkSlice) copyMatchingRows(from ...*Benchmark) {
	fromByPK := make(map[string]*Benchmark, len(from))
...
func (benchmark0 *Benchmark) InsertBenchmarkLog(ctx context.Context, exec bob.Executor, related *BenchmarkLogSetter) error {
	...
	benchmark0.R.Loaded.BenchmarkLog = true

	benchmarkLog1.R.Benchmark = benchmark0
	benchmarkLog1.R.Loaded.Benchmark = true
...
func (benchmark0 *Benchmark) InsertInstance(ctx context.Context, exec bob.Executor, related *InstanceSetter) error {
	...
	benchmark0.R.Loaded.Instance = true
...
func buildBenchmarkWhere[Q mysql.Filterable](cols benchmarkColumns) benchmarkWhere[Q] {
	return benchmarkWhere[Q]{
		...
		InstanceID:   mysql.Where[Q, string](cols.InstanceID.Expression),
		TeamID:       mysql.Where[Q, string](cols.TeamID.Expression),
		UserID:       mysql.Where[Q, string](cols.UserID.Expression),
		Status:       mysql.Where[Q, enums.BenchmarksStatus](cols.Status.Expression),
		CreatedAt:    mysql.Where[Q, time.Time](cols.CreatedAt.Expression),
		StartedAt:    mysql.WhereNull[Q, time.Time](cols.StartedAt.Expression),
		FinishedAt:   mysql.WhereNull[Q, time.Time](cols.FinishedAt.Expression),
		Score:        mysql.Where[Q, int64](cols.Score.Expression),
		Result:       mysql.WhereNull[Q, enums.BenchmarksResult](cols.Result.Expression),
		ErrorMessage: mysql.WhereNull[Q, string](cols.ErrorMessage.Expression),
		R:            benchmarkWhereR[Q]{cols: cols},
	}
}

// benchmarkWhereR holds the relationship-based filters of
// benchmarkWhere under the R namespace — mirroring the model's
// R struct — so they cannot collide with column-based filter fields.
type benchmarkWhereR[Q mysql.Filterable] struct {
	cols benchmarkColumns
}

// HasBenchmarkLog filters parents that have a matching BenchmarkLog using a
...
// applied to the subquery (i.e. to BenchmarkLogs).
func (w benchmarkWhereR[Q]) HasBenchmarkLog(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := mysql.Select(
		sm.Columns(mysql.Raw("1")),
		sm.From(BenchmarkLogs.NameExpr()),
		sm.Where(BenchmarkLogs.Columns.BenchmarkID.EQ(w.cols.ID)),
	)
	q.Apply(filters...)
	return mods.Where[Q]{E: mysql.Exists(q)}
}

// HasInstance filters parents that have a matching Instance using a
// correlated EXISTS subquery (semi-join). Unlike an INNER JOIN it does not
// multiply parent rows, so no DISTINCT is needed. The optional filters are
// applied to the subquery (i.e. to Instances).
func (w benchmarkWhereR[Q]) HasInstance(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := mysql.Select(
		sm.Columns(mysql.Raw("1")),
		sm.From(Instances.NameExpr()),
		sm.Where(Instances.Columns.ID.EQ(w.cols.InstanceID)),
	)
	q.Apply(filters...)
	return mods.Where[Q]{E: mysql.Exists(q)}
}

// NULL-tolerant scan buffer for benchmarkScanMapperNullable:
...
type benchmarkPreloadBuf struct {
	ID           sql.Null[string]
	InstanceID   sql.Null[string]
	TeamID       sql.Null[string]
	UserID       sql.Null[string]
	Status       sql.Null[enums.BenchmarksStatus]
	CreatedAt    sql.Null[time.Time]
	StartedAt    sql.Null[time.Time]
	FinishedAt   sql.Null[time.Time]
	Score        sql.Null[int64]
	Result       sql.Null[enums.BenchmarksResult]
	ErrorMessage sql.Null[string]
}

// benchmarkScanMapperNullable maps the preloaded benchmark
...
func benchmarkScanMapperNullable(prefix string) scan.Mapper[*Benchmark] {
	return func(ctx context.Context, cols []string) (scan.BeforeFunc, func(any) (*Benchmark, error)) {
		// resolve the column names once per query, not per row
		type target struct {
			idx int
			dst func(b *benchmarkPreloadBuf) any
		}
		targets := make([]target, 0, 11)
		for i, col := range cols {
			...
			switch name {
			case "id":
				targets = append(targets, target{i, func(b *benchmarkPreloadBuf) any { return &b.ID }})
			case "instance_id":
				targets = append(targets, target{i, func(b *benchmarkPreloadBuf) any { return &b.InstanceID }})
			case "team_id":
				targets = append(targets, target{i, func(b *benchmarkPreloadBuf) any { return &b.TeamID }})
			case "user_id":
				targets = append(targets, target{i, func(b *benchmarkPreloadBuf) any { return &b.UserID }})
			case "status":
				targets = append(targets, target{i, func(b *benchmarkPreloadBuf) any { return &b.Status }})
			case "created_at":
				targets = append(targets, target{i, func(b *benchmarkPreloadBuf) any { return &b.CreatedAt }})
			case "started_at":
				targets = append(targets, target{i, func(b *benchmarkPreloadBuf) any { return &b.StartedAt }})
			case "finished_at":
				targets = append(targets, target{i, func(b *benchmarkPreloadBuf) any { return &b.FinishedAt }})
			case "score":
				targets = append(targets, target{i, func(b *benchmarkPreloadBuf) any { return &b.Score }})
			case "result":
				targets = append(targets, target{i, func(b *benchmarkPreloadBuf) any { return &b.Result }})
			case "error_message":
				targets = append(targets, target{i, func(b *benchmarkPreloadBuf) any { return &b.ErrorMessage }})
			}
		}
		...
		buf := new(benchmarkPreloadBuf)
		return func(row *scan.Row) (any, error) {
				for _, t := range targets {
					row.ScheduleScanByIndex(t.idx, t.dst(buf))
				}
				return buf, nil
			}, func(link any) (*Benchmark, error) {
				buf := link.(*benchmarkPreloadBuf)
				...
				if !(buf.ID.Valid) &&
					!(buf.InstanceID.Valid) &&
					!(buf.TeamID.Valid) &&
					!(buf.UserID.Valid) &&
					!(buf.Status.Valid) &&
					!(buf.CreatedAt.Valid) &&
					!(buf.StartedAt.Valid) &&
					!(buf.FinishedAt.Valid) &&
					!(buf.Score.Valid) &&
					!(buf.Result.Valid) &&
					!(buf.ErrorMessage.Valid) {
					return nil, nil
				}
				...
				if buf.InstanceID.Valid {
					o.InstanceID = buf.InstanceID.V
				}
				if buf.TeamID.Valid {
					o.TeamID = buf.TeamID.V
				}
				if buf.UserID.Valid {
					o.UserID = buf.UserID.V
				}
				if buf.Status.Valid {
				...
				o.FinishedAt = buf.FinishedAt
				if buf.Score.Valid {
					o.Score = buf.Score.V
				}
				o.Result = buf.Result
				o.ErrorMessage = buf.ErrorMessage
				return o, nil
			}
	}
}
...
func (o *Benchmark) Preload(name string, retrieved any) error {
	...
	case "BenchmarkLog":
		...
		o.R.Loaded.BenchmarkLog = true

		if rel != nil {
			rel.R.Benchmark = o
			rel.R.Loaded.Benchmark = true
		}
		return nil
	case "Instance":
...
func buildBenchmarkPreloader() benchmarkPreloader {
	return benchmarkPreloader{
		BenchmarkLog: func(opts ...mysql.PreloadOption) mysql.Preloader {
			...
			}, BenchmarkLogs.Columns.Names(), benchmarkLogScanMapperNullable, opts...)
		},
		Instance: func(opts ...mysql.PreloadOption) mysql.Preloader {
			...
			}, Instances.Columns.Names(), instanceScanMapperNullable, opts...)
		},
	}
}
...
func (o *Benchmark) LoadBenchmarkLog(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	...
	related.R.Loaded.Benchmark = true
...
func (os BenchmarkSlice) LoadBenchmarkLog(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	...
	benchmarkByKey := make(map[string][]*Benchmark, len(os))
	for _, o := range os {
		if o == nil {
			continue
		}

		benchmarkByKey[o.ID] = append(benchmarkByKey[o.ID], o)
	}

	for _, rel := range benchmarkLogs {

		owners, ok := benchmarkByKey[rel.BenchmarkID]
		if !ok {
			continue
		}
...
func (os BenchmarkSlice) LoadInstance(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	...
	for _, o := range os {
		...
		benchmarkByKey[o.InstanceID] = append(benchmarkByKey[o.InstanceID], o)
	}

	for _, rel := range instances {

		owners, ok := benchmarkByKey[rel.ID]
		if !ok {
			continue
		}
	...
	return benchmarkJoins[Q]{
		...
		BenchmarkLog: modAs[Q, benchmarkLogColumns]{
			...
			f: func(to benchmarkLogColumns) bob.Mod[Q] {
				...
				{
					mods = append(mods, dialect.Join[Q](typ, BenchmarkLogs.NameExpr().As(to.Alias())).On(
		...
		Instance: modAs[Q, instanceColumns]{
			...
			f: func(to instanceColumns) bob.Mod[Q] {
				...
				{
					mods = append(mods, dialect.Join[Q](typ, Instances.NameExpr().As(to.Alias())).On(
