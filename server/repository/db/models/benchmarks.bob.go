...
func (s *BenchmarkSetter) Apply(q *dialect.InsertQuery) {
	...
	q.AppendValues(bob.ExpressionFunc(func(ctx context.Context, w io.Writer, d bob.Dialect, start int) ([]any, error) {
		...
		if s.UserID.IsUnset() {
			...
			vals[3] = mysql.Arg(s.UserID)
		}

		if s.Status.IsUnset() {
			vals[4] = mysql.Raw("DEFAULT")
		} else {
			vals[4] = mysql.Arg(s.Status)
		}
		...
		if s.StartedAt.IsUnset() {
			...
			vals[6] = mysql.Arg(s.StartedAt)
		}

		if s.FinishedAt.IsUnset() {
			vals[7] = mysql.Raw("DEFAULT")
		} else {
			vals[7] = mysql.Arg(s.FinishedAt)
		}

		if s.Score.IsUnset() {
			vals[8] = mysql.Raw("DEFAULT")
		} else {
			vals[8] = mysql.Arg(s.Score)
		}

		if s.Result.IsUnset() {
			vals[9] = mysql.Raw("DEFAULT")
		} else {
			vals[9] = mysql.Arg(s.Result)
		}
